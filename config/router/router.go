...
func Init(r *gin.Engine) {
	...
	// 设置路由
	{
		...
		r.GET("/reg", user.Reg_Get)                                                //注册页面
		api.POST("/verify", user.SendEmailCode)                                    //获取验证码
		api.POST("/reg", user.Reg_Post)                                            //注册操作
		r.GET("/index", middleware.TokenMiddleware, controllers.SendHomePageHtml)  //进入表白墙主页
		r.GET("/space", middleware.TokenMiddleware, controllers.SendSpacePageHtml) //进入个人空间，区分管理员和普通用户
